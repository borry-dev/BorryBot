	...
	async def eightball(self, ctx, arg=None):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			if arg == None:
				...
				embed = discord.Embed( title = "🔮 Магический шар", description = random.choice(answers_rus), color = config.MAIN_COLOR )
				await ctx.send( embed = embed )
		else:
			if arg == None:
				await ctx.send( embed = discord.Embed( title = "You didn't enter the question!", color = config.ERR_COLOR ) )
			else:
				embed = discord.Embed( title = "🔮 Magic Ball", description = random.choice(answers_eng), color = config.MAIN_COLOR )
				await ctx.send( embed = embed )
