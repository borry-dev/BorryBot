	...
	async def on_command_error(self, ctx, err):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if isinstance(err, errors.CommandNotFound):
				...
				emb = discord.Embed( title = "Missing permissions!", description = f"You don't have permission to execute this command!", color= config.ERR_COLOR)
				emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
				await ctx.send( embed = emb )
			elif isinstance(err, commands.errors.NSFWChannelRequired):
				emb = discord.Embed( title = "Error!", description = f"The use of this command is allowed only in NSFW channels!", color = config.ERR_COLOR)
				emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
				await ctx.send( embed = emb )
			elif isinstance(err, commands.CommandOnCooldown):
				emb = discord.Embed( title = "You have a cooldown!", description = f"Your cooldown failed! Try again later!", color = config.ERR_COLOR)
				emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
				await ctx.send( embed = emb )
			elif isinstance(err, commands.errors.MemberNotFound):
				...
				emb = discord.Embed( title = "Error!", description = f"You didn't specify an argument!", color = config.ERR_COLOR )
				emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
				await ctx.send( embed = emb )
			else:
				channel = self.client.get_channel(config.ERR_CHANNEL)
				await ctx.send( embed = discord.Embed( title = "Unknown error!", description = f"An unknown error has occurred: `{err}`\nThe error has already been sent to the developer to fix it", color= config.ERR_COLOR) )
