    ...
    async def lang(self, ctx, lang=None):
        if lang is None:
            if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
                emb = discord.Embed( title = 'На сервере установлен русский язык!', description = f'Для смены языка введите `{config.PREFIX}lang [rus/eng]`', color = config.MAIN_COLOR )
                emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
                await ctx.send(embed=emb)
            else:
                emb = discord.Embed( title = 'English is installed on the server!', description = f'To change the language, enter `{config.PREFIX}lang [rus/eng]`', color = config.MAIN_COLOR )
                emb.set_footer(text=f'{self.client.user.name} | {config.YEAR}', icon_url=self.client.user.avatar_url)
                await ctx.send(embed=emb)
        else:
            if lang == 'rus':
                ...
                if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
                    emb = discord.Embed( title = 'Выбран недоступный язык!', description = '`rus` - установить русский язык\n`eng` - установить английский язык', color = config.MAIN_COLOR )
                    emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
                    await ctx.send( embed = emb )
                else:
                    emb = discord.Embed( title = 'An unavailable language is selected!', description = '`rus` - install the Russian language\n`eng` - install the English language', color = config.MAIN_COLOR )
                    emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
                    await ctx.send( embed = emb )
