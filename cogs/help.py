...
class Help(commands.Cog):
	...
	async def help(self, ctx):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if ctx.invoked_subcommand == None:
				emb = discord.Embed( title = 'Commands', description = f'To view the commands, enter `{config.PREFIX}help [module name]`', color = config.MAIN_COLOR )
	...
	async def moderation(self, ctx):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb.add_field( name = f'{config.PREFIX}ban <member ping> [reason]' , value = 'Ban a member on the server', inline = False)
			emb.add_field( name = f'{config.PREFIX}kick <member ping> [reason]', value = 'Kick a member on the server', inline = False )
			emb.add_field( name = f'{config.PREFIX}clear [number of messages(max. 50)]', value = 'Clear a certain number of messages', inline = False )
			emb.set_thumbnail( url = self.client.user.avatar_url )
			emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
			await ctx.send( embed = emb )
	...
	async def support(self, ctx):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb.add_field( name = f'{config.PREFIX}bug [the essence of the bug]' , value = 'Send a bug to the bot developer', inline = False)
			emb.add_field( name = f'{config.PREFIX}idea [your idea]', value = 'Send an idea to the bot developer', inline = False )
