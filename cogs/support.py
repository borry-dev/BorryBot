...
class Support(commands.Cog):
	...
	async def bug(self, ctx, *, bug=None):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if bug is None:
				await ctx.send( embed = discord.Embed( title = "Error!", description = "You didn't specify a bug!", color= config.MAIN_COLOR ) )
			else:
				channel = self.client.get_channel(config.BUG_CHANNEL)
				...
				emb.set_footer( text = f"Username:{ctx.author.name}#{ctx.author.discriminator}\nID:{ctx.author.id}", icon_url = ctx.author.avatar_url )
				await channel.send( embed = emb )
				embed = discord.Embed( title = 'The bug has been sent!',description = f'Bug: {bug}',color = config.MAIN_COLOR )
				embed.set_thumbnail( url = self.client.user.avatar_url )
				embed.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
				await ctx.send( embed = embed )
	...
	async def idea(self, ctx, *, idea=None):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if idea is None:
				await ctx.send( embed = discord.Embed( title = "Error!", description = "You didn't specify the idea!", color= config.MAIN_COLOR ) )
			else:
				channel = self.client.get_channel(config.IDEA_CHANNEL)
				...
				emb.set_footer( text = f"Username:{ctx.author.name}#{ctx.author.discriminator}\nID:{ctx.author.id}", icon_url = ctx.author.avatar_url )
				await channel.send( embed = emb )
				embed = discord.Embed( title = 'The idea has been sent!',description = f'Idea: {idea}',color = config.MAIN_COLOR )
				embed.set_thumbnail( url = self.client.user.avatar_url )
				embed.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
				await ctx.send( embed = embed )
	...
	async def invite(self, ctx):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb = discord.Embed( title = ':link: | Invite a bot', description = f'[Invite](https://discord.com/oauth2/authorize?client_id={self.client.user.id}&permissions={config.SCOPE}&scope=bot)', color = config.MAIN_COLOR )
			emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
			await ctx.send( embed = emb )
