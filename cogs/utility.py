...
class Utility(commands.Cog):
	...
	async def calc(self, ctx, *, exp = None):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if exp is None:
				await ctx.send( embed = discord.Embed( title = "Error!", description = f"Enter a mathematical example", color = config.ERR_COLOR ) )
			else:
				link = 'http://api.mathjs.org/v4/'
				...
				try:
					...
					await ctx.send( embed = discord.Embed( title = "Error!", description = f"You can't use text in the mathematical example", color = config.ERR_COLOR ) )
		...
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if member is None:
				for activity in ctx.author.activities:
					if isinstance(activity, Spotify):
						emb = discord.Embed( title = 'Spotify', color = config.MAIN_COLOR )
						emb.add_field( name = 'Title:', value = f'[{activity.title}](https://open.spotify.com/track/{activity.track_id})', inline = False )
