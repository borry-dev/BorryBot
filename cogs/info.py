...
class Info(commands.Cog):
	...
	async def info(self, ctx):
		if cursor_language.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb = discord.Embed( title = f'{self.client.user.name}#{self.client.user.discriminator}', description = 'Information about the **BorryBot**', color = config.MAIN_COLOR )
			...
			emb.add_field( name = 'Invite a bot to the server:', value = '[Invite](https://discord.com/api/oauth2/authorize?client_id=862195560019001344&permissions=8&scope=bot)' )
			emb.add_field( name = 'Support Server:', value = '[Join](https://discord.gg/FJRj894gUn)' )
			emb.add_field( name = 'Useful links:', value = '[bots.server-discord.com](https://bots.server-discord.com/862195560019001344)\n[Boosty](https://boosty.to/borrybot)' )
			emb.set_thumbnail( url = self.client.user.avatar_url )
			emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
			await ctx.send( embed = emb )
	...
	async def stats(self, ctx):
		if cursor_language.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb.add_field( name = 'Completed commands:', value = f"""`{cursor.execute("SELECT count FROM stats WHERE name = 'commands'").fetchone()[0]}`""", inline = True)
			emb.add_field( name = 'Number of commands:', value = f'`{total_commands}`', inline = True )
			emb.add_field( name = 'Number of modules:', value = f'`{total_modules}`', inline = True )
	...
	async def user(self, ctx, member:discord.Member=None):
		if cursor_language.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if member is None:
				...
				emb.add_field( name = 'ID:', value = f'{ctx.author.id}', inline = False )
				emb.add_field( name = 'The member has registered:', value = ctx.author.created_at.strftime('%d.%m.%Y %H:%M:%S'), inline = False )
				...
				emb.add_field( name = 'ID:', value = f'{member.id}', inline = False )
				emb.add_field( name = 'The member has registered:', value = member.created_at.strftime('%d.%m.%Y %H:%M:%S'), inline = False )
	...
	async def server(self, ctx):
		if cursor_language.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			emb.add_field( name = 'Online:', value = len({m.id for m in ctx.guild.members if m.status is not discord.Status.offline}), inline = False )
			emb.add_field( name = 'Channels:', value = f'{len(ctx.guild.channels)} ({len(ctx.guild.text_channels)} text channels | {len(ctx.guild.voice_channels)} voice channels) ', inline = False )
	...
	async def vote(self, ctx):
		if cursor_language.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			emb = discord.Embed( title = 'Мониторинги', description = 'bots.server-discord.com: [Перейти](https://bots.server-discord.com/862195560019001344)\nboticord.top: [Перейти](https://boticord.top/bot/862195560019001344)', color = config.MAIN_COLOR )
			emb.set_thumbnail( url = self.client.user.avatar_url )
			emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
			await ctx.send( embed = emb )
		else:
			emb = discord.Embed( title = 'Monitoring', description = 'bots.server-discord.com: [Перейти](https://bots.server-discord.com/862195560019001344)\nboticord.top: [Перейти](https://boticord.top/bot/862195560019001344)', color = config.MAIN_COLOR )
			emb.set_thumbnail( url = self.client.user.avatar_url )
			emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
			await ctx.send( embed = emb )
