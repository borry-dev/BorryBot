	...
	async def clear( self, ctx, amount:int=None ):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if amount is None:
				...
				if amount > 50:
					emb = discord.Embed( title = 'Error!', description = 'You can delete a maximum of 50 messages at a time', color = config.ERR_COLOR )
					emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
					await ctx.send( embed = emb )
				elif amount <= 0:
					emb = discord.Embed( title = 'Error!', description = 'The bot cannot restore commands!', color = config.ERR_COLOR )
					emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
					await ctx.send( embed = emb )
				else:
					await ctx.channel.purge( limit = amount + 1 )
					emb = discord.Embed( title = 'Clear', description = f'{ctx.author.mention} deleted {amount} messages', color = config.MAIN_COLOR )
					emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
					await ctx.send( embed = emb )
