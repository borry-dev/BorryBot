...
class Settings(commands.Cog):
	...
	async def slowmode(self, ctx, sec:int = None):
		if cursor.execute(f"SELECT guild_id FROM english WHERE guild_id = {ctx.guild.id}").fetchone() is None:
			...
			if sec is None:
				emb = discord.Embed( title = 'Error!', description = f'Specify the duration of the slow mode', color = config.ERR_COLOR )
				await ctx.send( embed = emb )
			else:
				if sec > 0:
					await ctx.channel.edit( slowmode_delay = sec )
					emb = discord.Embed( title = 'Slow mode is enabled!', description = f'For the channel `{ctx.channel.name}` slow mode is set to `{sec}s`', color = config.MAIN_COLOR )
					emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
					await ctx.send( embed = emb )
				if sec < 0:
				...
				if sec == 0:
					await ctx.channel.edit( slowmode_delay = sec )
					emb = discord.Embed( title = 'Slow mode is disabled!', description = f'For the channel `{ctx.channel.name}` slow mode disabled', color = config.MAIN_COLOR )
					emb.set_footer( text = f'{self.client.user.name} | {config.YEAR}', icon_url = self.client.user.avatar_url )
					await ctx.send( embed = emb )
